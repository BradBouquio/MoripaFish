...
class YamlRecordDao(
    ...
    override fun update(record: Record) {
        ...
        setRecord(yaml.getConfigurationSection(id) ?: throw IllegalStateException("Couldn't get configuration section"), record)
        yaml.save(file)
    }
    ...
    override fun all(): List<Record> {
        ...
        for (section in yaml.getKeys(false).map(yaml::getConfigurationSection)) {
            ...
            val fish = Fish(fishType, fishLength ?: throw IllegalStateException("Illegal fish type"))
